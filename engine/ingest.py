...
def fetch_historical(start: date, end: date) -> dict:
    """Historical bundle for calibration. Weather from the ERA5 archive
    ...
    try:
        ...
        try:
            ...
            r.raise_for_status()
            mr = r.json()
            warnings.append("Marine history limited to ~92 past days; calibration uses the overlap window.")
        except Exception as exc:
            warnings.append(f"Marine history unavailable ({exc}); calibration would be weather-only; aborting is wiser.")

    return {"source": "open-meteo-archive", "forecast": fc, "marine": mr, "warnings": warnings}
